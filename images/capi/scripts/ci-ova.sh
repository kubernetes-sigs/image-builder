...
TARGETS=( $(make build-node-ova-vsphere-all --recon -d | grep "Must remake" | grep -v build-node-ova-vsphere-all | grep -E -v 'rhel|windows|efi' | grep -E -o 'build-node-ova-vsphere-[a-zA-Z0-9\-]+' ) )
